...
class CenticService {
    ...
    public async getAAVEV3AprOnArbitrum(usdcAddress: string): Promise<any> {
        usdcAddress = usdcAddress.toLowerCase();

        const url = "https://develop.centic.io/dev/v3/projects/lending/aave-v3/overview?chain=0xa4b1";
        ...
        try {
            ...
            const asset = data.markets[0].assets.find((m: any) => m.address == usdcAddress);

            return asset ? asset.supplyAPY * 100 : 0;
        } catch (error) {
            console.error("Error fetching AAVE V3 APR:", error);
            throw error;
        }
    }
